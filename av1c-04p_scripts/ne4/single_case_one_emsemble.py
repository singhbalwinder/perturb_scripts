...
main_case      = 'ne4_av1c04p2_const_int'        #case name without init cond file names
...
cam_hist_str   = '.cam.h0.'
suffix         ='.nc'
ts             = '00000' #Time stamp on history files ['07200','01800','00000']
...
ymin  = 0 #-1    # Use 'N' to unset it 
ymax  = 10.0 #10.0  # Use 'N' to unset it 
xmin  = 0
...
fvar.close()
inic_list = list(map(str.strip,inic_list))

#get xticklabel
with open(xlabel_file, 'r') as flbl:
   xlabels = flbl.readlines()
flbl.close()
xlabels = list(map(str.strip,xlabels))
...
for icase in inic_list:   
   ...
   yr_mn_dy = list(reversed(inic_list[i].rsplit('.')))[1].split('-00000')[0] #now reversing the string after splitting
   ...
   for iprt in test_case_str:
      ...
      ax.semilogy(res_nzero,linestyle='-',marker='.', linewidth=2, color=clr)
      ax.set_xlim([0,len(res_nzero)])
      ax.set_xticks(range(0,len(res_nzero)))
      ax.set_xticklabels(xlabels_nzero, rotation=90, ha='center', fontsize=12)
      ax.set_xlabel('Physical processes in the first time step', fontsize=12)
